 ...
 * @param {string} inColGoogleName The column letter where the name of the person is.
 * @param {string} inColGoogleName2 The column letter where the name if the company is.
 * @param {string} outColGoogleName The column letter where the results should be written.
 */
function startGoogleNameProcessing(
  startRowGoogleName = 2,
  endRowGoogleName = 100,
  inColGoogleName = 'C',
  inColGoogleName2 = 'F',
  outColGoogleName = 'AI'
) {
  ...
  if (currentendRowGoogleName > endRowGoogleName)
    currentendRowGoogleName = endRowGoogleName;
  ...
  processGoogleNameBatches(
    startRowGoogleName,
    currentendRowGoogleName,
    inColGoogleName,
    inColGoogleName2,
    outColGoogleName
  );
 ...
 * @param {string} outColGoogleName The column letter where the results should be written.
 */
function processGoogleNameBatches(
  startRowGoogleName,
  endRowGoogleName,
  inColGoogleName,
  inColGoogleName2,
  ...
  prompts.forEach((row, index) => {
    if (row[0] !== '') {
      try {
        let response = GoogleName(row[0], prompts2[index][0]);
  ...
  const outputRange = `${outColGoogleName}${startRowGoogleName}:${outColGoogleName}${
    startRowGoogleName + results.length - 1
  }`;
...
function continueGoogleNameProcessing() {
  ...
  if (nextendRowGoogleName > endRowGoogleName)
    nextendRowGoogleName = endRowGoogleName;

  // Process the next batch
  processGoogleNameBatches(
    startRowGoogleName,
    nextendRowGoogleName,
    inColGoogleName,
    inColGoogleName2,
    outColGoogleName
  );
  ...
  properties.setProperty(
    'lastRowProcessedGoogleName',
    nextendRowGoogleName.toString()
  );
